FPS = 60
GAME_NAME = 'Pong 1.5'
POINT_LIMIT = 10  # Playing to and included

BACKGROUND_COLOR = 47, 50, 58
PADDLE_COLOR = 255, 255, 255
BALL_COLOR = 255, 255, 255
TEXT_COLOR = 255, 255, 255

PADDLE_HEIGHT = 100
PADDLE_WIDTH = 15
PADDLE_LIMIT = 8  # pixels from top of surface to the bar
PADDLE_SPEED = 10

BALL_SIZE = 10
BALL_START_SPEED = 5
BALL_SPEED_INCREMENT = 1

# text
GAME_DEV = 'Laget av Jonas'
START_TEXT = 'Trykk mellomrom for å starte'
SETTING_TEXT = 'Bruk piltastene for å endre instillinger'

# Ikke brukt enda
CONNECTION_AWAIT_STATUS_TEXT = 'Venter på en connection'
CONNECTION_AWAIT_HINT_TEXT = 'Trykk ESC for å gå tilbake'

EASY_TEXT = '< Nybegynner >'
HARD_TEXT = '< Vrien >'
IMPOSSIBLE_TEXT = '< Vanskelig (umulig) >'

ONE_PLAYER = '^ 1-spiller ^'
TWO_PLAYER_LOCAL = '^ 2-spiller: Lokal ^'
TWO_PLAYER_SOCKET = '^ 2-spiller: Socket ^'

PAUSE_TEXT = 'Spill pauset!'
UNPAUSE_TEXT = 'Trykk ESC for å fortsette'
PAUSE_EXIT = 'Trykk Enter for å avslutte'
