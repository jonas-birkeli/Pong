...
class Ball:
    ...
    def calc_real_vel(self) -> None:
        screen_width = self.game_values.get_screen_size()[0]
    ...
    def move(self) -> None:
        ...
        if self.pos_x > screen_size[0] - constant.BALL_SIZE:
            self.set_winner(1)

            return

        # Bounce from roof or ground
        if self.pos_y < 0 or self.pos_y + constant.BALL_SIZE > screen_size[1]:
            self.vel_y *= -1
